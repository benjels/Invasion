...
public class GameCanvas extends Canvas{
	...
	public void paint(Graphics g){
		if(this.tiles != null && this.entities != null){//TODO: gross solution to only painting if we havent initialised the tiles
		...
		for(int row = 0; row < tiles.length; row++){
			for(int col = 0; col < tiles.length; col++){
				...
				if(tile != null){ //@Joely note that there should not be any nulls in the program (from my code). the lack of some object is indicated by a "NullInstance" type object e.g. NullEntity
					Image tileImage = Imagehelper.loadImage2("stone64.png");//TODO: hardcoded only drawing stones
					if(false){//TODO: "tile instanceof WallTile"
						g.drawImage(tileImage, xOffset + point.x, yWallOffset + point.y, null,null);
					} else {
						g.drawImage(tileImage, xOffset + point.x, yOffset + point.y, null,null);
					}

				}//HARDCODED GROSS SHIT FOR TESTING PLAYER MOVEMENT/MAX (just drawing a different tile to represnet the player
